...
class Group(Scene):
    ...
    # Calls intersect() on self.objs, updates hit
    def intersect(self, hit, ray):
        l = self.b.ray(ray)
        if l >= hit.llambda:
            return
    ...
    for dx in range(0, ss):
        for dy in range(0, ss):
            ...
            g += ray_trace(Vector(-1.0, -3.0, 2.0).unitise(),
                           ray, scene)
    print('Value ' + str(int(0.5 + 255.0 * g / ss ** 2)) +
          ' writing to: ' + str(chnl.name))
